"""Abbiamo addestrato una semplice rete neurale con un set più ampio 
di dati sui prezzi di appartamenti. La rete prevede il prezzo 
dell'appartamento in base agli attributi della cabina. 
La rete è composta da un livello di input con cinque nodi, 
un livello nascosto con due nodi, un secondo livello nascosto 
con due nodi e infine un livello di output con un singolo nodo. 
Inoltre, c'è un singolo nodo di bias per ogni livello nascosto e 
il livello di output.

Il programma sottostante usa i pesi di questa rete addestrata 
per eseguire quello che viene chiamato un forward pass della rete neurale. 
Il forward pass esegue le variabili di input attraverso la rete neurale 
per ottenere un output, in questo caso il prezzo di un appartamento."""
import numpy as np
